...
pub fn derive_typed_command_builder(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
        ...
        syn::Data::Struct(data) => match &data.fields {
            ...
            syn::Fields::Unnamed(_) => {
                return Err(Error::new(
                    ast.span(),
                    "TypedCommandBuilder is not supported for tuple structs",
                ))
            }
            syn::Fields::Unit => {
                return Err(Error::new(
                    ast.span(),
                    "TypedCommandBuilder is not supported for unit structs",
                ))
            }
        },
        syn::Data::Enum(_) => {
            return Err(Error::new(
                ast.span(),
                "TypedCommandBuilder is not supported for enums",
            ))
        }
        syn::Data::Union(_) => {
            return Err(Error::new(
                ast.span(),
                "TypedCommandBuilder is not supported for unions",
            ))
        }
    };
    Ok(data)
